...
from utils.dataset import BasicDataset, get_list, preprocess_img_gt, crop_tile, CROP_FLAG
...
def predict_netOnDataList(root_dir, net, val_list, device, mask_threhold, vali, result_path, vis_flag=VISILE):
    ...
    for i, fn in enumerate(val_list):
        ...
        jaccard_i, pred_mask, bi_mask, bi_contour = predict_img(net=net,
                                                    full_img=img,
                                                    gt=gt,
        ...
        if vis_flag:
            ...
            cv2.imwrite(os.path.join(vali_path, '%d_4bi_vis.jpg' % i), (bi_mask * 255).astype(np.uint8))
            cv2.imwrite(os.path.join(vali_path, '%d_5bi_contour_vis.jpg' % i), (bi_contour * 255).astype(np.uint8))
            cv2.imwrite(os.path.join(vali_path, '%d_2gt_vis.jpg' % i), gt_vis.astype(np.uint8))
            cv2.imwrite(os.path.join(vali_path, '%d_3pred_vis_%.4f.jpg' % (i,jaccard_i)), pred_vis.astype(np.uint8))
...
if __name__ == "__main__":
    ...
    # model_paths = [[os.path.join(root_path, 'val 0_CP_epoch196_scoreMask_1.6782_scoreContour_0.4565.pth'), 0]]
    model_paths=[[os.path.join(root_path, mod), int(mod[4])] for mod in os.listdir(root_path) if mod.find('.pth') != -1]
    model_paths=sorted(model_paths, key=lambda x:int(re.findall(r"\d+",x[0])[1]))
    model_paths=[model_paths[115-1]] #115
