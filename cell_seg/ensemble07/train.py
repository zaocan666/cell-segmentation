...
def train_net(net,
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=0, pin_memory=True, worker_init_fn=np.random.seed(random_seed))
    ...
    scheduler = optim.lr_scheduler.ExponentialLR(optimizer, gamma=np.power(0.01, 1/epochs))
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                masks_pred = net(imgs)
                loss = criterion(masks_pred, true_biMasks)
                ...
                if global_step % ((n_train+n_val) // (10 * batch_size)) == 0:
                    ...
                    val_score = eval_net(net, val_loader, device, n_classes)
                    ...
                    if n_classes > 1:
                        logging.info('val {} Validation cross entropy: {}'.format(val_i, val_score))
                        writer_main.add_scalar('val_%d_Loss/test'%val_i, val_score, global_step)
                    else:
                        logging.info('val {} Validation score: {}'.format(val_i, val_score))
                        if val_score<10:
                            writer_main.add_scalar('val_%d_score/test'%val_i, val_score, global_step)

                    writer_detail.add_images('val_%d_images'%val_i, imgs[0:2], global_step)
                    if n_classes == 1:
                        writer_detail.add_images('val_%d_masks/true'%val_i, true_biMasks[0:2], global_step)
                        writer_detail.add_images('val_%d_masks/pred'%val_i, torch.sigmoid(masks_pred[0:2]) > 0.5, global_step)
        scheduler.step()
...
def get_args():
    ...
    parser.add_argument('-e', '--epochs', metavar='E', type=int, default=45,
