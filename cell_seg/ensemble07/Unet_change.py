...
class up_conv(nn.Module):
    ...
    def forward(self,x, x2_shape):
        x1 = self.up(x)

        diffY = x2_shape[2] - x1.size()[2]
        diffX = x2_shape[3] - x1.size()[3]
...
class AttU_Net(nn.Module):
    ...
    def forward(self,x):
        ...
        x2 = self.Maxpool(x1) #314
        x2 = self.Conv2(x2)
        
        x3 = self.Maxpool(x2) #157
        x3 = self.Conv3(x3)

        x4 = self.Maxpool(x3) #78
        x4 = self.Conv4(x4)

        x5 = self.Maxpool(x4) #39
        x5 = self.Conv5(x5)

        # decoding + concat path
        d5 = self.Up5(x5, x4.shape)
        x4 = self.Att5(g=d5,x=x4)
        d5 = torch.cat((x4,d5),dim=1)        
        d5 = self.Up_conv5(d5)
        
        d4 = self.Up4(d5, x3.shape)
        x3 = self.Att4(g=d4,x=x3)
        d4 = torch.cat((x3,d4),dim=1)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4, x2.shape)
        x2 = self.Att3(g=d3,x=x2)
        d3 = torch.cat((x2,d3),dim=1)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3, x1.shape)
