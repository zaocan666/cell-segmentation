...
def get_list(dataset_dir, val_i):
    ...
    with open(dataset_dir, 'r') as f:
        ...
        for l in lines:
            strs = l.strip().split(' ')
            val_i_true=int(strs[2])
            if val_i_true==val_i:
                val_list.append([strs[0], strs[1]])
            else:
                train_list.append([strs[0], strs[1]])
...
def preprocess_img_gt(img, gt, train_flag):
    img[img>255]=255
...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        img, gt = preprocess_img_gt(img, gt, self.train_flag)
